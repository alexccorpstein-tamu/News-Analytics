...
database_name = 'T:\\Jamestestconnection.db'
...
try:
    ...
    while(looping):
        answer = str(input("What would you like to do:\n\nCREATE TABLE(C), LIST OF TABLES(L) FILL (F), READ RAW DATA (RR), READ ANALYZED DATA (RA), OR DELETE TABLE (D): "))
        
        if answer != 'C' and answer != 'L' and answer != 'l' and answer != 'F' and answer != 'RA' and answer != 'RR' and answer != 'D' and answer != 'rr' and answer != 'c' and answer != 'f' and answer != 'ra' and answer != 'd':
    ...
    ###########READ RAW FUNCTION##################################
            ...
            table_name = str(input("What table of raw data would you like to read from: "))
            
            read_df = pd.DataFrame(columns = ["Headline", "Date", "Article", "Publisher", "neg", "neu", "pos", "compound", "sentiment", "subjectivity"])
            ...
            for row in cursor.execute("SELECT rowid, * FROM {}".format(table_name)):
                ...
                insert_row = pd.Series([headline, date, article, publisher, neg, neu, pos, compound, sentiment, subjectivity], index = read_df.columns)
